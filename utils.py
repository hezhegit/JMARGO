...
class Ontology(object):
    ...
    def __init__(self, filename='data/go.obo', with_rels=False):
        # 加载给定文件中的本体数据，并可选择是否包含关系信息
        self.ont = self.load(filename, with_rels)
        self.ic = None

    # 检查给定的 term_id 是否存在于本体中
    def has_term(self, term_id):
        return term_id in self.ont

    # 获取给定 term_id 的详细信息，如果 term_id 存在于本体中则返回其详细信息，否则返回 None。
    ...
    # 计算信息内容（IC）值。
    # IC 值是一种表示概念信息丰富程度的指标，在本体学习中经常被用来衡量术语的重要性。
    # 该方法接受注释（annots）作为参数，并根据这些注释计算每个术语的 IC 值。
    ...
    # 获取给定 term_id 的信息内容（IC）值。如果 IC 值已经计算，则返回相应的 IC 值，否则抛出异常。
    ...
    # 从文件中加载本体数据。该方法解析给定文件中的本体信息，并返回一个包含本体术语及其详细信息的字典。
    ...
    # 获取给定 term_id 的所有祖先术语。它使用广度优先搜索算法来遍历本体图，找到给定术语的所有祖先术语。
    ...
    # 获取给定 term_id 所属的命名空间。
    def get_namespace(self, term_id):
        return self.ont[term_id]['namespace']

    # 获取给定 term_id 的所有子孙术语。它使用广度优先搜索算法来遍历本体图，找到给定术语的所有子孙术语。
