...
class CNN1DDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # protbert
        protbert_path ='/home/415/hz_project/MMSMAPlus-master/data/cafa_feats'
        protbert = pickle.load(open(protbert_path + '/' + name + '.pkl', 'rb'))['bert']
        features_protbert = protbert.astype(np.float32).T
        ...
        return features_bert, seqlen, labels, features_esm2, features_protbert
...
def cnn1d_collate(batch):
    ...
    protbert = [item[4] for item in batch]
    ...
    truncated_pb_feats = []
    for feat in protbert:
        if feat.shape[1] > MAXLEN:
            truncated_pb = feat[:, :MAXLEN]
        else:
            truncated_pb = feat
        truncated_pb_feats.append(truncated_pb)
    protbert = truncated_pb_feats
    ...
    feats_pad_pb = [np.pad(item, ((0, 0), (0, max_len - lengths[i])), 'constant') for i, item in enumerate(protbert)]

    # else
    x3 = torch.from_numpy(np.array(feats_pad_pb))

    return CustomData(x1=x1, x2=x2,x3=x3, y=torch.from_numpy(np.array(labels)))
